...
Function which send message to clients when someone new has connected to the server.
Reads all bits for filedescriptor. */
void BroadCastToAllClients(fd_set activeFdSet, int socket, char newclientName)
{
  int i = 0;
  char *Message[MAXMSG];
  sprintf(Message, "Client %s has connected to the server.", newclientName);

  for(; i<socket; i++)
  {
    //FD_ISSET Checks if socket(i) is in our bit for the file descriptor(activeFdSet), if true then send message about newclient to socket(i). 
    ...
    for(i = 0; i < FD_SETSIZE; ++i) 
      if(FD_ISSET(i, &readFdSet)) 
      {
      	 if(i == sock) 
         {
          ...
          {//If connection is successfull broadcast to all client that new client has connected.
        	  printf("Server: Connect from client %s, port %d\n", 
        		 inet_ntoa(clientName.sin_addr), 
        		 ntohs(clientName.sin_port));
        	  FD_SET(clientSocket, &activeFdSet);

            BroadCastToAllClients(activeFdSet, sock, clientName.sin_addr);
          ...
          else //If we have received an incoming message and we want to reply.
          {
            char messageString[20] = "I hear ya!"; 
